#!/bin/bash
##
# Based on: install-hyprland-overrides.sh
# Original author: typecraft-dev
# Source: https://github.com/typecraft-dev/omarchy-supplement
#
# This version:
...
JETBRAINS_CONFIG="$SCRIPT_DIR/jetbrains.conf"
...
# Check of hyprland-overrides.conf dependencies
if [ ! -f "$JETBRAINS_CONFIG" ]; then
  echo "Jetbrains config not found at $JETBRAINS_CONFIG"
  exit 1
fi
